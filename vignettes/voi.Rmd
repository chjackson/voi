...
Note the result is slightly different from `evpi1`, since to get that, we used a value of exactly 1 for the expected net benefit under current information, because we knew the exact distribution of the net benefit given current information was normal with mean 1.  In realistic models, we will need to compute this mean as an average over the random sample.  In this example, this mean is estimated as `r max(colMeans(outputs_nb))`, which leads to a slightly less accurate estimate than the one based on a known mean of 1. 
...
(from [Strong et al.](https://doi.org/10.1177/0272989X13505910)).  The number of random samples to use in this computation can be changed using the `nsim` argument, which can be useful for this method as it can be prohibitive for large samples.  Here the sample of 10000 is reduced to 1000. 
