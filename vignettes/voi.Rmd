...
While some details may not be fully explained in the current draft of this document, the `voi` package is intended to accompany a book which is under preparation.  When the book is finished, it will provide all background information that users of the package will need to know.  As the book is developed, useful features may be added to the package. 

This document gives a simple overview of how the package is used.  A simple example model is used, but the same methods work in more complex models.

The example model is a model used for _decision making_, which has been the most common application of VoI, e.g. in health economic evaluations.   A later section describes the use of VoI methods for a model that is used for _estimation_ of uncertain quantities, rather than for explicit decision-making. 
...
# evppivar(p2, inputs, pars=list("p1", "beta")) # ideally this would work, but isn't currently implemented!
```
...
sqrt(var(p2) - evppi_beta)
sqrt(var(p2) - evppi_p1)
```

Hence we would reduce the SD of $p_1$ to around 2/3 of its original value by learning $p_1$ or $\beta$.


### How regression-based EVPPI estimation works

EVPPI is estimated here by _nonparametric regression_ of the output on the input.  This idea came from [Strong et al.](https://dx.doi.org/10.1177%2F0272989X15575286) in the context of models with decisions. [Jackson et al. 2019](https://doi.org/10.1080/01621459.2018.1562932) showed how it applied in a wider class of problems, including models for estimation. 
...
```{r}
evsivar(p2, inputs, study = "binary", pars="p1", n=100)
evsivar(p2, inputs, study = "binary", pars="p1", n=1000)
evsivar(p2, inputs, study = "binary", pars="p1", n=10000)
```
...
evsivar(p2, inputs=inputs_p2, study = "binary", pars="p2", n=100)
evsivar(p2, inputs=inputs_p2, study = "binary", pars="p2", n=1000)
evsivar(p2, inputs=inputs_p2, study = "binary", pars="p2", n=10000)
```
...
In a future version of `voi` it would be nice to allow the user to calculate EVSI for a vector of alternative sample sizes `n` in a single call to `evsi` or `evsivar`. 
