...
```{r,echo=FALSE}
chemo_dir <- setwd(system.file("Chemotherapy_Book",package="voi"))
```
...
nb <- as.data.frame(m_costs_effects[,1,]*20000 - m_costs_effects[,2,])
```
...
par_groups <- list(
  ...
  "trans_probs" = c("p_hospitalised_total","p_died","lambda_home","lambda_hosp"),
...
```{r,eval=FALSE}
evppi(outputs=nb, inputs=m_params, pars=list("p_side_effects_t2","u_hospital"))
evppi(outputs=nb, inputs=m_params, pars=list("p_side_effects_t2","u_hospital"), method="earth")
```
